...
def main():
    ...
    #修改，增加参数，train-error-dir，更改了随机种子的图做损失训练
    new_run_parser.add_argument('--train-error-dir', default='/public/wangxiangkun/chat-gan-grey/gen1000_7.4981/train', type=str,
                                help='The directory where the data for error is stored.')
    new_run_parser.add_argument('--valid-error-dir', default='/public/wangxiangkun/chat-gan-grey/gen1000_7.4981/valid', type=str,
                                help='The directory where the data for error valid is stored.')
    #修改
    ...
    new_run_parser.add_argument('--batch-size', type=int, help='The batch size.', default=4)
    ...
    #修改，增加lambda_e=1
    lambda_adv, lambda_g, lambda_p, lambda_m, lambda_e = 1, 1, 0, 100, 1
    ...
    train_loader = DataLoader(train_dataset, batch_size=args.batch_size, drop_last=True, shuffle=False, **kwargs)
    ...
    #修改
    error_dataset = utils.Mydataset(args.train_error_dir)
    error_loader = DataLoader(error_dataset, batch_size=args.batch_size, drop_last=True, shuffle=False, **kwargs)
    valid_error_dataset = utils.Mydataset(args.valid_error_dir)
    valid_error_loader = DataLoader(valid_error_dataset, batch_size=args.batch_size, drop_last=True, shuffle=False, **kwargs)
    #修改
    ...
    if args.command == 'continue':
        ...
        #修改，增加error_loss，mse_ce
    metric_names = ['adv_loss', 'mse_loss', 'error_loss','vgg_loss', 'decoder_loss', 'loss',
                    'bit_err', 'decode_accuracy','decode_accuracy2', 'psnr', 'ssim', 'mse_ce']
    ...
    for e in range(start_epoch, args.epochs):
        ...
        while i < len(train_loader):
            ...
            if use_discriminator:
                ...
                while j < Diters and i < len(train_loader):
                    ...
                    #修改
                    error_in = next(error_iter)
                    #修改
                    while  mse_loss(image_in, error_in) > 20:
                        image_in = next(train_iter)
                        error_in = next(error_iter)
                        i += 1
                    ...
                    if args.cuda:
                        ...
                        error_in = error_in.cuda()
                    #修改

                    optimizer_discriminator.zero_grad()
                    stego = encoder(image_in, message_in)
		            #修改
                    d_on_cover = discriminator(error_in)
            ...
            g_mse_loss = mse_loss(stego, image_in)  #mse损失定义
            #修改，增加g_error_loss定义
            g_error_loss = mse_loss(stego, error_in)
            #修改
            g_mse_ce = mse_loss(image_in, error_in)
            #修改
            ...
            #修改
            g_loss = (lambda_adv * g_adv_loss + lambda_g * g_mse_loss + lambda_m * g_decoder_loss + lambda_p * g_vgg_loss + lambda_e * g_error_loss) \
                if use_discriminator else (lambda_g * g_mse_loss + lambda_m * g_decoder_loss + lambda_p * g_vgg_loss + lambda_e * g_error_loss)
            g_loss.backward()
            #修改
            ...
            with torch.no_grad():
                ...
                #修改
                metrics['error_loss'] += g_error_loss.item()
                metrics['mse_ce'] += g_mse_ce.item()
                #修改
		        ...
		        #修改，psnr,ssim改为和error_in
                metrics['psnr'] += utils.psnr_between_batches(error_in, stego_round)
                metrics['ssim'] += pytorch_ssim.ssim(error_in / 255.0, stego_round / 255.0).item()
        ...
        discriminator.eval()
        #修改
        error_iter = iter(valid_error_loader)
        #修改
        for batch_id, image_in in enumerate(valid_loader):
            ...
            error_in = next(error_iter)
            #修改
            #if  mse_loss(image_in, error_in) > 20:
            #    continue
            if args.cuda:
                message_in = message_in.cuda()
                image_in = image_in.cuda()
            ...
            with torch.no_grad():
                ...
                #修改，增加g_error_loss定义
                g_error_loss =mse_loss(stego_round, error_in)
                g_mse_ce = mse_loss(image_in, error_in)
                #修改
                g_vgg_loss = torch.tensor(0.)
                if args.use_vgg:
                ...
                #修改
                val_metrics['error_loss'] += g_error_loss.item()
                val_metrics['mse_ce'] += g_mse_ce.item()
                #修改
                ...
                #修改
                val_metrics['psnr'] += utils.psnr_between_batches(error_in, stego_round)
                val_metrics['ssim'] += pytorch_ssim.ssim(error_in / 255.0, stego_round / 255.0).item()
